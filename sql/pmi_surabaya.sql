...
(1, 'Surabaya', '(031) 5313289', 'Jl. Embong Ploso No.7-15, Embong Kaliasin, Kec. Genteng, Kota SBY, Jawa Timur 60271');
...
('PD-SBY001', 6, 1, 'AB', '2023-02-08', 2),
('PD-SBY002', 5, 1, 'AB', '2023-02-08', 3),
('PD-SBY003', 5, 1, 'AB', '2023-02-08', 2);
...
CREATE TRIGGER `tg_tb_dd_insert` BEFORE INSERT ON `tb_distribusi_darah` FOR EACH ROW BEGIN
  ...
  SET NEW.nomor_distribusi = CONCAT('PD-SBY', LPAD(LAST_INSERT_ID(), 3, '0'));
END
$$
DELIMITER ;
...
(13, 'Leo Uisetiawan', 'AB'),
(14, 'Bagas Farenheit', 'B');
...
('RM-SBY001', 1, 13, 'AB', '2023-02-07'),
('RM-SBY002', 1, 14, 'B', '2023-02-07'),
('RM-SBY003', 1, 13, 'AB', '2023-02-07');
...
CREATE TRIGGER `tg_tb_rm_insert` BEFORE INSERT ON `tb_rekam_medis` FOR EACH ROW BEGIN
  ...
  SET NEW.nomor_rekam_medis = CONCAT('RM-SBY', LPAD(LAST_INSERT_ID(), 3, '0'));
END
$$
DELIMITER ;
...
(5, 'RS Al-Irsyad', '(031) 99091800', 'Jalan KH Mas Mansyur No.210-214 dan 191, Nyamplungan, Kec. Pabean Cantikan, Kota SBY, Jawa Timur 60162'),
(6, 'RS Adi Husada Kapasari', '(031) 3764555', 'Jl. Kapasari No.97-101, Kapasan, Kec. Simokerto, Kota SBY, Jawa Timur 60141');
